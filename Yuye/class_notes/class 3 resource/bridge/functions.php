...
if (!function_exists('qode_styles')) {
    function qode_styles() {
		...
		if (file_exists(dirname(__FILE__) ."/css/custom_css.css") && qode_is_css_folder_writable())
        	wp_enqueue_style("custom_css", QODE_ROOT . "/css/custom_css.css", array(), filemtime(dirname(__FILE__) ."/css/style_dynamic_responsive.css"));
       	else
...
if (!function_exists('qode_scripts')) {
    function qode_scripts() {
        ...
        if(isset($qode_toolbar)):
            wp_enqueue_script("toolbarInfiniteScroll", QODE_ROOT."/js/toolbarInfiniteScroll.js",array(),false,true);
...
if(!function_exists('qode_localize_no_ajax_pages')) {
	...
	function qode_localize_no_ajax_pages() {
		...
		if(qode_is_ajax_enabled()) {
			...
			$no_ajax_pages = array_merge($no_ajax_pages, qode_get_posts_without_ajax());

            //get pages that have ajax disabled and merge with main array
			$no_ajax_pages = array_merge($no_ajax_pages, qode_get_pages_without_ajax());
...
if(!function_exists('qode_get_posts_without_ajax')) {
	/**
	 * Function that returns urls of posts that have ajax disabled
	 * @return array array of urls of posts that have ajax disabled
	 *
	 * @version 0.1
	 */
	function qode_get_posts_without_ajax() {
